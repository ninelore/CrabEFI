#!/bin/bash
# Run CrabEFI in QEMU with AHCI/SATA storage
#
# Usage: ./scripts/run-qemu-ahci.sh [coreboot.rom] [disk.img]
#
# Prerequisites:
...
echo "=== CrabEFI QEMU Test (AHCI/SATA) ==="
...
# Run QEMU with Q35 machine (has ICH9 AHCI controller built-in)
QEMU_ARGS=(
    -machine q35
    -bios "$COREBOOT_ROM"
    -m 512M
...
# Attach disk to AHCI controller
# Q35 has ICH9-AHCI on bus 0, slot 0x1f, function 2
QEMU_ARGS+=(
