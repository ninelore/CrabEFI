...
use crate::efi::varstore::{persist_variable, VarStoreError};
...
fn persist_key_variable(var_type: SecureBootVariable, data: &[u8]) -> Result<(), AuthError> {
    ...
    // Persist to SMMSTORE
    persist_variable(&guid, name, SECURE_BOOT_KEY_ATTRS, data).map_err(|e| {
...
pub fn clear_all_keys() -> Result<(), AuthError> {
    ...
    let _ = persist_key_variable(SecureBootVariable::PK, &[]);
    let _ = persist_key_variable(SecureBootVariable::KEK, &[]);
    let _ = persist_key_variable(SecureBootVariable::Db, &[]);
    let _ = persist_key_variable(SecureBootVariable::Dbx, &[]);
